#!/usr/bin/env python3
"""
改良版 Cosmos DB → Elasticsearch 全件同期スクリプト
重複防止、エラーハンドリング、チェックポイント機能を強化
"""
import asyncio
import logging
import sys
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'improved_sync_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
...
async def main():
    """改良版全件データの同期を実行"""
    print("\n" + "="*70)
    print("🚀 改良版 Cosmos DB → Elasticsearch 全件同期")
    print("="*70)
    print("✨ 新機能:")
    print("   - 重複防止機能")
    print("   - 強化されたエラーハンドリング")
    print("   - チェックポイントからの自動復旧")
    print("   - データ整合性チェック")
    print("   - バッチ処理の最適化")
    print("="*70)
    ...
    try:
        ...
        if checkpoint:
            ...
            print(f"   - 処理済みバッチ: {checkpoint.get('batch_num', 0)}")
            if 'stats' in checkpoint:
                stats = checkpoint['stats']
                print(f"   - 処理済みドキュメント: {stats.get('documents_processed', 0):,}件")
                print(f"   - インデックス済み: {stats.get('documents_indexed', 0):,}件")
        ...
        await orchestrator.sync_all_documents(
            force_recreate_index=False,  # インデックスを再作成しない（既存データを保持）
            resume_from_checkpoint=True   # チェックポイントから復旧
        )
        ...
        print("\n\n⚠️  処理が中断されました")
        print("チェックポイントが保存されているため、再実行時に続きから処理できます")
        sys.exit(1)
        ...
        print("チェックポイントが保存されている場合、再実行時に続きから処理できます")
        sys.exit(1)


if __name__ == "__main__":
    print("\n" + "🚀"*35)
    print("改良版 Cosmos DB → Elasticsearch 全件同期ツール")
    print("🚀"*35)
