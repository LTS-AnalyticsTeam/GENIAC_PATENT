...
load_dotenv()
# ===== Azure Search設定 =====
SEARCH_ENDPOINT = os.environ['AZURE_SEARCH_ENDPOINT']  # 環境変数から取得
SEARCH_KEY = os.environ['COSMOS_ENDPOINT']  # 環境変数から取得
ENHANCED_INDEX_NAME = "enhanced_theme_code_index"

# キーが設定されていない場合の警告
if not SEARCH_KEY:
    print("AZURE_SEARCH_KEY environment variable is not set. Azure Search functionality will be disabled.")
    SEARCH_KEY = None

# ===== Azure OpenAI設定 =====
AOAI_ENDPOINT = os.environ['AZURE_SEARCH_ENDPOINT']
AOAI_KEY = os.environ['AZURE_OPENAI_SEARCH_KEY'] # 環境変数から取得

# キーが設定されていない場合の警告
if not AOAI_KEY:
    print("AZURE_OPENAI_SEARCH_KEY environment variable is not set. Azure OpenAI functionality will be disabled.")
    AOAI_KEY = None
...
if AOAI_KEY:
    ...
    print("Azure OpenAI client not initialized (no API key)")
...
def embed_text(text: str) -> List[float]:
    """テキストをベクトルに変換"""
    if not aoai_client:
        print("Azure OpenAI client not available for embedding")
        return []
    try:
        # 長すぎるテキストは切り詰める
        if len(text) > 8000:
