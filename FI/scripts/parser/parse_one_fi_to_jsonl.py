...
class FIClassificationParser:
        ...
        try:
            ...
            for row in rows:
                ...
                if code is None:
                    ...
                    if m_num:
                        code = expand_code(m_num.group(0), link["name"].strip() if (link and link.has_attr("name")) else None)
    ...
    def explore_directory(self, directory: Path, max_depth: int = 10, current_depth: int = 0):
        ...
        if index_file.exists():
            ...
            if result:
                ...
                print(f"Parsed: {index_file.relative_to(self.root_dir)}")
...
def build_nodes(pages: List[Dict]) -> Dict[str, Dict]:
    ...
    for page in pages:
        ...
        for item in page["classifications"]:
            code = item["code"]
            title = j_normalize(item.get("title") or "")  # タイトルが無ければ空
            desc = j_normalize(item.get("description") or "")
            level = infer_level(code)
            section = code[0] if code else ""

            # タイトルが空なら、可能なら description の短縮を使う
            if not title:
                # descriptionの先頭句をタイトル扱いにする簡易策
                title = desc.split("（", 1)[0].split(" ", 1)[0][:120] if desc else ""
            ...
            if not node:
                node = {
                    ...
                    # RAG用テキストを後から追加するならここに text_leaf/text_context を入れる
                }
                nodes[code] = node
            else:
                # 重複行が来たら、descriptionを長い方/情報量が多い方に更新するなどのポリシー
...
def main():
    ap = argparse.ArgumentParser(description="FI HTML → JSONL 変換")
    ap.add_argument("--root", default=".", help="FI ルートディレクトリ")
    ap.add_argument("--max-depth", type=int, default=12, help="最大探索深度")
    ap.add_argument("--jsonl-out", default="FI.jsonl", help="出力JSONLパス")
    ap.add_argument("--section", default="", help="特定セクションのみ出力（例: A）空なら全体")
    args = ap.parse_args()

    parser = FIClassificationParser(args.root)
    parser.explore_directory(Path(args.root), args.max_depth)

    nodes = build_nodes(parser.pages)
    out_path = Path(args.jsonl_out)
    write_jsonl(nodes, out_path, section_filter=(args.section or None))

    print(f"Pages parsed: {len(parser.pages)}")
    print(f"Nodes written: {sum(1 for _ in out_path.open(encoding='utf-8'))}")
